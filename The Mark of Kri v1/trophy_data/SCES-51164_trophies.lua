...
local H5 =
	function()
		...
		local unlock = eeObj.GetGPR(gpr.a2)
...
local H12 =-- Count kills in single hit
	function()
		local hitpoints = eeObj.ReadMemFloat(eeObj.GetGPR(gpr.a2) + 0xa54) -- get object's hp
		local bodyhit = eeObj.ReadMem32(eeObj.GetGPR(gpr.a2) + 0x88) -- 1 = body, 0 = armor -- bugfix 8644(b) 20150630 TGL.
		local hitcount = eeObj.GetGPR(gpr.s4) -- get hit number
		...
		if hitcount == 1 and hitpoints <= 0 and bodyhit == 1 then -- hit 1 is a kill -- bugfix 8644(b) 20150630 TGL.
			if killed == 0 then -- we didn't count this one yet
				killed = 1
			end
		end
		if hitcount == 2 and hitpoints <= 0 and bodyhit == 1 then -- hit 2 is a kill -- bugfix 8644(b) 20150630 TGL.
			if killed == 1 then -- we didn't count this one yet
				killed = 2
			end
		end
		if hitcount == 3 and hitpoints <= 0 and bodyhit == 1 then -- hit 3 is a kill -- bugfix 8644(b) 20150630 TGL.
			if killed == 2 then -- we didn't count this one yet
				local level = eeObj.ReadMem32(eeObj.ReadMem32(gLevelManager) + 0x7c)
				level = level - 2
...
Hook12 = eeObj.AddHook(0x16f2f0, 0x26940001, H12) -- collider_checkAttackCollisionVsAllObj__FP9ObjHandle
