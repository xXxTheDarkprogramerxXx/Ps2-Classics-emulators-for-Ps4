...
-- Bug#8948
local kethor_exists = false
local kethor_obj_address = 0
local std_string_c_str = function(ptr)
   return eeObj.ReadMemStr(eeObj.ReadMem32(eeObj.ReadMem32(ptr) + 12))
end
eeObj.AddHook(0x0016cec0, 0x27bdfb50, function()
				 local str1 = std_string_c_str(eeObj.GetGpr(gpr.a1))
				 if string.find(string.lower(str1), "kethor", 1, true) then
					-- print(string.format("Command : %s", str1))
				 end
				 if str1 == "doitem M08_G147_ORS, kethor" then
					-- print("Kethor exists")
					kethor_exists = true
					kethor_obj_address = 0
				 elseif str1 == "set kethor.hitpoints = 0" then
					-- print("Kethor Died")
					kethor_exists = false
				 end
end)
eeObj.AddHook(0x2b5430,	0x27b100b8, function()
				 local obj   = eeObj.ReadMem32(eeObj.ReadMem32(eeObj.GetGpr(gpr.s5)+120)+92)

				 -- only when Kethor object
				 if kethor_exists and obj == kethor_obj_address then
					-- print("Kethor moves forcibly!")
					local sp = eeObj.GetGpr(gpr.sp)
					-- Kethor's coordinate
					local x  = eeObj.ReadMemFloat(obj+52)
					local y  = eeObj.ReadMemFloat(obj+56)
					local z  = eeObj.ReadMemFloat(obj+60)
					-- Shield Generator's one
					local tx = -1663.260010
					local ty = -198.119003
					local tz = -1600.0
					-- Put recalculated vector onto the stack.
					eeObj.WriteMemFloat(sp+176, tx - x)
					eeObj.WriteMemFloat(sp+180, ty - y)
					eeObj.WriteMemFloat(sp+184, tz - z)
				 end
end)
eeObj.AddHook(0x464724, 0x7bb00000, function()
				 if kethor_obj_address == 0 then
					kethor_obj_address = eeObj.ReadMem32(eeObj.GetGpr(gpr.v0)+92)
					-- print(string.format("Kethor Object = %x", kethor_obj_address))
				 end
end)
...
--Disable fire up-render for performance reason. 
 local thresholdArea = 40 --Total triangle area that triggers optimization ( 1 area unit = 320x240 pixels)  
